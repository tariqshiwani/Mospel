...
using Mospel.Serializer;
using Mospel.MqttBroker;

namespace Mospel.MqttSerializer
{
    public class MqttPacketSerializer
    {
        ...
        public static MqttBasePacket Deserialize(byte[] data)
        {
            int i = 0;
            int PacketType = (data[i] >> 4);
            int BodyLength = 0;
            i++;
            byte newByte = 0;
            int multiplier = 1;
            ...
            {
                newByte = data[i];
                BodyLength += (newByte & 127) * multiplier;
                multiplier *= 128;
            ...
            data.Skip(i);
            byte[] body = data.Take(BodyLength).ToArray();
            Array.Copy(data, i, body, 0, BodyLength);
            MqttPacketBodyReader BodyReader = new MqttPacketBodyReader(body, 0, body.Length);
            MqttBasePacket packet;

            switch ((MqttControlPacketType)PacketType)
            {
                case MqttControlPacketType.Connect:
                    packet = DeserializeConnect(BodyReader);
                    break;
                case MqttControlPacketType.ConnAck:
                    packet = DeserializeConnAck(BodyReader);
                    break;
                case MqttControlPacketType.Disconnect:
                    packet = new MqttDisconnectPacket();
                    break;
                case MqttControlPacketType.Publish:
                    packet = DeserializePublish(data[0], BodyReader);
                    break;
                case MqttControlPacketType.PubAck:
                    packet = DeserializePubAck(BodyReader);
                    break;
                case MqttControlPacketType.PubRec:
                    packet = DeserializePubRec(BodyReader);
                    break;
                case MqttControlPacketType.PubRel:
                    packet = DeserializePubRel(BodyReader);
                    break;
                case MqttControlPacketType.PubComp:
                    packet = DeserializePubComp(BodyReader);
                    break;
                case MqttControlPacketType.PingReq:
                    packet = new MqttPingReqPacket();
                    break;
                case MqttControlPacketType.PingResp:
                    packet = new MqttPingRespPacket();
                    break;
                case MqttControlPacketType.Subscribe:
                    packet = DeserializeSubscribe(BodyReader);
                    break;
                case MqttControlPacketType.SubAck:
                    packet = DeserializeSubAck(BodyReader);
                    break;
                case MqttControlPacketType.Unsubscibe:
                    packet = DeserializeUnsubscribe(BodyReader);
                    break;
                case MqttControlPacketType.UnsubAck:
                    packet = DeserializeUnsubAck(BodyReader);
                    break;
                default: throw new MqttProtocolViolationException($"Packet type ({PacketType}) not supported.");
            }

            packet.ControlPacketType = (MqttControlPacketType)PacketType;
        ...
        private static MqttBasePacket DeserializePublish(byte FixedHeader, MqttPacketBodyReader body)
        {
            ThrowIfBodyIsEmpty(body);
        ...
        private static MqttBasePacket DeserializeConnAck(MqttPacketBodyReader body)
        {
            ThrowIfBodyIsEmpty(body);
        ...
        private byte Serialize(MqttConnectPacket packet, MqttPacketWriter packetWriter)
        {
            ...
            if (packet.ProtocolVersion == MqttProtocolVersion.V311)
            {
